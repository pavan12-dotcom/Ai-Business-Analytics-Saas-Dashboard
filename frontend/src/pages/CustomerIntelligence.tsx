...
export default function CustomerIntelligence() {
  ...
  const healthData = useMemo(() => {
    if (hasData && kpis.length >= 3) {
      // Build radar from available KPI distribution
      const metricKpi = kpis.find(k => k.rawValue > 0)
      const base = metricKpi ? Math.min(95, Math.max(40, (metricKpi.rawValue / (metricKpi.rawValue * 1.2)) * 95)) : 75
      return [
        { subject:'Engagement',   A: Math.round(base * 0.92) },
        { subject:'Retention',    A: Math.round(base * 0.88) },
  ...
  const ttvData = useMemo(() => {
    if (hasData && monthly.length >= 3) {
      // Simulate activation curve from monthly revenue trend
      const total = monthly.reduce((s, m) => s + m.revenue, 0)
      return monthly.slice(0, 7).map((m, i) => ({
        day: m.month,
        activated: total > 0 ? Math.round(100 - (i * (100 / (monthly.length + 1)))) : 100 - i * 10
      }))
    }
    return MOCK_TTV
  }, [hasData, monthly])
  ...
  const cohortData = useMemo(() => {
    if (hasData && primaryTimeKey && rawRows.length > 0) {
      // Group rows by quarter of primaryTimeKey
      const quarterMap: Record<string, {size:number; dateKey:Date}> = {}
      rawRows.forEach((r: any) => {
        const rawDate = r[primaryTimeKey]; if (!rawDate) return
        const dt = cleanDateColumn([rawDate]).validDates[0]; if (!dt) return
  ...
  return (
    <div className="int-page fade-in">
      ...
      <div className="int-kpi-row">
        <StatPill
          label={hasData ? `Total ${entityName}s` : 'Total Customers'}
        ...
        <StatPill
          ...
          change={hasData && atRiskCount > 0 ? '+2.1%' : '+2.1%'} up={false}
        />
        <StatPill
          label="Churn Rate"
      ...
      <div className="int-grid-2">
          ...
          <div className="int-card-header">
            <div>
              <div className="int-card-title">
                {hasData ? `${entityName} Segments` : 'Customer Segments'}
              </div>
        ...
        <div className="int-card">
          <div className="int-card-header">
            <div>
              <div className="int-card-title">Health Score Radar</div>
              <div className="int-card-sub">Aggregate health across 6 dimensions</div>
            </div>
          </div>
      ...
      <div className="int-grid-2">
        <div className="int-card">
          <div className="int-card-header">
            <div>
              <div className="int-card-title">Churn Risk by Segment</div>
              <div className="int-card-sub">% at churn risk per category</div>
            </div>
          </div>
          <div className="int-chart-wrap">
            ...
            <div>
              ...
              <div className="int-card-sub">
                {hasData && primaryTimeKey
                  ...
                  : '% customers who activated within N days'}
              </div>
            </div>
          </div>
          <div className="int-chart-wrap">
      ...
      <div className="int-card">
        <div className="int-card-header">
          <div>
            <div className="int-card-title">Cohort Retention Heatmap</div>
            <div className="int-card-sub">
              {hasData && primaryTimeKey
